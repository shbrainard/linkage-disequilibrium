...
LD.thin <- read.table(file = "data/dean-d10-thin-1000-ldwindow-100.ld", sep="\t", header=TRUE, stringsAsFactors = F)


# Split big matrix into list separated into chromosomes
LD.by.chroms.thin <- splitByChrom(LD.thin, nchrom = 9, sep = "")

# Loop over chromosomes and get the mean r2 in each window (either sliding or non-overlapping)
LD.by.window.thin <- slidingWindow(LD.by.chroms.thin, windowSize = 100)

LD.by.window.thing <- nonOverlappingWindow(LD.by.chroms.thin, windowSize = 100)


#save(LD.by.window, file = "r2SlidingWindow.RData")

allchroms <- LD.by.window.thin[[10]]
